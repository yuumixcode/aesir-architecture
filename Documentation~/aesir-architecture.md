...
> **版本**: 0.1.0 | **Unity**: 2022.3+ | **命名空间**: `Runestone.AesirArchitecture`
...
- [4. 核心概念](#4-核心概念)
  ...
  - [4.6 MiniEventHub — 类型事件总线](#46-minieventhup--类型事件总线)
...
- **Model → View**：通过 `ObservableProperty` 值变更通知或 `MiniEventHub` 事件推送
...
> `MiniEvent` 不通过 `MiniEventHub` 管理，适合独立使用的简单无参事件场景。
...
```csharp
// 属性
public MiniEventHub EventHub { get; }
...
- [x] MiniEventHub 类型事件总线 + 自动注销生命周期绑定
- [x] Domain Reload 安全
- [x] MockContext 测试隔离
