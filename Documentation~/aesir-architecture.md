...
| **运行时错误日志** | `GetModel<T>()` / `GetService<T>()` 在目标未注册时抛出含调用者类型和目标类型信息的异常 |
| **AbstractSubmodule 统一生命周期** | Model 和 Service 的公共生命周期逻辑提取到 `AbstractSubmodule` 基类 |
| **Domain Reload 安全** | 静态变量通过 `[RuntimeInitializeOnLoadMethod]` 显式重置，反复进出 Play Mode 无残留 |
| **纯 C# 核心 + MonoBehaviour 适配** | 框架核心为纯 C# 对象，`AesirView<T>` / `MonoView<T>` / `AesirViewController<T>` 作为 MonoBehaviour 适配层 |
...
> **设计原则**：IService 不包含 ICanExecuteCommand（Command 应由 Controller/Presenter 触发），IController 不包含 ICanAddListener（事件监听应由 Service 或 View 负责）。
...
- **Domain Reload 安全**：`GenericLocator<T>` 的静态构造函数自动注册到 `ResetStaticsAssistant`，在 `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` 时重置全局引用

> **注意**：`AbstractContext<T>` 是泛型类型，无法直接使用 `[RuntimeInitializeOnLoadMethod]`。因此通过非泛型的 `ResetStaticsAssistant` 统一管理所有静态变量的域重置回调。回调列表不在重置后清空，因为关闭 Domain Reload 时静态构造函数不会重新执行，回调需保留供下次进入 Play Mode 使用。
...
View + Controller 双角色合一的基类，适用于 MVC 模式下 View 直接执行命令的场景。
...
继承后自动获得 GetModel、GetService、ExecuteCommand 能力，无需单独创建 Controller。
...
> Controller 定位为命令执行入口，不包含 `ICanAddListener`（事件监听应由 Service 或 View 负责）。
...
{
    public static MiniEventBus Global { get; }          // 全局单例
...
- `GenericLocator<T>` 的静态构造函数自动注册到 `ResetStaticsAssistant`
